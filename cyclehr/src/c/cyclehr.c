...
static void prv_inbox_received(DictionaryIterator *iter, void *context) {
  ...
  // Only mark the weather as valid when the message actually carried weather.
  // pkjs pushes the stored settings on 'ready', before the first forecast has
  // been fetched — without this guard that config-only message would light up
  // the default sun icon next to an empty "--°C".
  if (temp || cond || icon) {
...
static void prv_window_load(Window *window) {
  ...
  // bpm top = HR text bottom - bpm font_size (24 emery / 12 diorite).
